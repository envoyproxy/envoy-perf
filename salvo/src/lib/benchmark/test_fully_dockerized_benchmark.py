...
from src.lib.benchmark import (fully_dockerized_benchmark as full_docker, base_benchmark)
...
def test_execute_benchmark_with_envoy_source(mock_run_image):
  """Validate that if sources are defined that enable us to build the Envoy image we do not throw \
    an exception."""
  mock_run_image.return_value = b'benchmark_http_client output'
